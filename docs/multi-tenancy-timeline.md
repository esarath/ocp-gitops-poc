...
| 3 | Implement `ResourceQuota` + `LimitRange` | `base/{stage,prod}/{resourcequota,limitrange}.yaml` | Day 2 |
...
| 7 | Deploy to cluster (ArgoCD sync), run post-deployment checks, sign off | Synced `stage`/`prod` namespaces, completed validation checklist | Day 6, green CI |
...
(cluster sync + validation) is the explicit next step, gated on the CI run
passing and your go-ahead, since it changes live cluster state.
